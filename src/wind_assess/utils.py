...
Helper utilities: power curve loading, Weibull/wind-rose plotting.
"""
...
def load_power_curve_csv(path: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    For your uploaded files, the columns are e.g.:
    - "Wind Speed [m/s]"
    - "Power [kW]"

    This function tries to find the appropriate columns automatically.
    """
    df = pd.read_csv(path)

    # Try to detect wind speed column
    speed_col = None
    power_col = None
...
def plot_weibull_fit(speeds: np.ndarray, k: float, A: float, ax=None):
    """
    Plot histogram of speeds and fitted Weibull PDF.
...
def plot_wind_rose(directions_deg: np.ndarray, ax=None, n_sectors: int = 16):
    ...
    directions_deg : np.ndarray
        Wind direction [deg] where 0 = North, 90 = East.
    n_sectors : int
    ...
    dirs = np.deg2rad(directions_deg)
