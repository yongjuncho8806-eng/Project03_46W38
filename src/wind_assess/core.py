...
class WindResource:
    ...
    def _speed_dir_from_uv(u: xr.DataArray, v: xr.DataArray) -> Tuple[xr.DataArray, xr.DataArray]:
        """
        Compute wind speed [m/s] and direction [deg] from u, v components.

        Direction is in meteorological convention:
        - 0 deg = North
        - 90 deg = East
        - 180 deg = South
        - 270 deg = West
    ...
    def get_speed_direction_at_point(
        ...
        speed : xarray.DataArray
            Wind speed [m/s] with time dimension.
        direction : xarray.DataArray
            Wind direction [deg] with time dimension.
        """
    ...
    def estimate_alpha_at_point(
        ...
        -------
        alpha : float
            Median alpha over the selected time period.
        """
        ds_sub = self._subset_years(start_year, end_year)
        ds_pt = self._interp_point(ds_sub, lat, lon)
    ...
    def extrapolate_speed_power_law(
        ...
        ----------
        lat, lon : float
            Location inside the ERA5 grid.
        z_target : float
            Target height [m].
        z_ref : float, optional
            Reference height [m] (default 100 m).
        alpha : float, optional
            If None, estimated from 10m & 100m speeds at the point.
        start_year, end_year : int
            Period to use.

        Returns
        -------
        speed_target : xarray.DataArray
            Wind speed at z_target [m/s] with time dimension.
        """
        if alpha is None:
            alpha = self.estimate_alpha_at_point(lat, lon, start_year, end_year)

        # reference speed at z_ref: use 10m or 100m according to z_ref
        if int(z_ref) == 10:
    ...
    def fit_weibull_1d(speeds: np.ndarray) -> Tuple[float, float]:
        ...
        if speeds.size == 0:
            raise ValueError("Cannot fit Weibull to empty speed array.")

        c, loc, scale = weibull_min.fit(speeds, floc=0.0)
    ...
    def fit_weibull_at_point(
        ...
        lat, lon : float
        height : float
            If height is 10 or 100 and use_power_law=False, use ERA5 directly.
            Otherwise, speeds are extrapolated using power law from 100m.
        start_year, end_year : int
        use_power_law : bool
            If True and height != 10 or 100, speeds are extrapolated.
    ...
    def compute_aep_from_power_curve(
        ...
        """
        Compute AEP (MWh) for a given turbine power curve at a point and year.
        ...
        speed_bins : np.ndarray
            Wind speed values [m/s] where power curve is defined.
        power_kw : np.ndarray
            Corresponding turbine power [kW].
        year : int
            Year within 1997–2008 to use.
        availability : float
            Turbine availability factor (0–1). Default 1.0.
